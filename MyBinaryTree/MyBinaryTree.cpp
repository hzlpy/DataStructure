...
int main(void)
{
	...
	printf("levelTraverseBinaryTree: \n");
	levelTraverseBinaryTree(pTree);
	printf("\n");
...
void levelTraverseBinaryTree(PBTNODE pRoot)
{
	...
	q.push(pRoot);
	while (!q.empty()) {
		PBTNODE frontNode = q.front();
		q.pop();
		printf("%c ",frontNode->data);
		if (frontNode->pLeftChild != NULL) {
			q.push(frontNode->pLeftChild);
		}
		if (frontNode->pRightChild != NULL) {
			q.push(frontNode->pRightChild);
		}
	}
	return;
}
